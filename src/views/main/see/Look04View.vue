...
onMounted(() => {
  ...
  // 计算有可及性和无可及性的数据总和
  const withAccessibilitySum = xiaomai1.filter(item => item.Prob >= 0.5).reduce((acc, item) => acc + item.Length, 0);
  const withoutAccessibilitySum = xiaomai1.filter(item => item.Prob < 0.5).reduce((acc, item) => acc + item.Length, 0);
  const scatterOption = {
    title: {
      left: 'center',
      text: 'probability'
    ...
    series: [
      {
        ...
        id: 'total',
        data: [
          withAccessibilitySum,
          withoutAccessibilitySum
        ],
